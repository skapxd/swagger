...
describe('Express Swagger', () => {
    ...
    let builder;
    beforeEach(async () => {
        app = await core_1.NestFactory.create(app_module_1.ApplicationModule, new platform_express_1.ExpressAdapter(), { logger: false });
    ...
    describe('disabled Swagger UI but served JSON/YAML definitions', () => {
        ...
        it('should serve the YAML definition file', async () => {
            const response = await request(app.getHttpServer()).get(`${SWAGGER_RELATIVE_URL}-yaml`);
            expect(response.status).toEqual(200);
    ...
    describe('custom documents endpoints with global prefix', () => {
        ...
        const YAML_CUSTOM_URL = '/apidoc-yaml';
        beforeEach(async () => {
            appGlobalPrefix = await core_1.NestFactory.create(app_module_1.ApplicationModule, new platform_express_1.ExpressAdapter(), { logger: false });
    ...
    describe('custom swagger options', () => {
        ...
        beforeEach(async () => {
            const swaggerDocument = lib_1.SwaggerModule.createDocument(app, builder.build());
            lib_1.SwaggerModule.setup('/', app, swaggerDocument, {
        ...
        it('should patch the OpenAPI document', async () => {
            const response = await request(app.getHttpServer()).get('/swagger-ui-init.js?description=Custom%20Swagger%20description%20passed%20by%20query%20param');
        ...
        it('should patch the OpenAPI document based on path param of the swagger prefix', async () => {
            const app = await core_1.NestFactory.create(app_module_1.ApplicationModule, new platform_express_1.ExpressAdapter(), { logger: false });
            const swaggerDocument = lib_1.SwaggerModule.createDocument(app, builder.build());
            lib_1.SwaggerModule.setup('/:customer/', app, swaggerDocument, {
                patchDocumentOnRequest(req, res, document) {
                    return {
                        ...document,
                        info: {
            ...
            await app.init();
            const response = await request(app.getHttpServer()).get('/customer-1/swagger-ui-init.js');
            await app.close();
